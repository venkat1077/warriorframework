...
def execute_testsuite(testsuite_filepath, data_repository, from_project,
    ...
    3. execution_dir        = (string) the full path of the directory under which the testsuite execution directory will be created 
                                       (results for the testsuite will be stored in the  testsuite execution directory.)
    """
    ...
    if execution_type.upper() == 'PARALLEL_TESTCASES':
        ...
        Max_Attempts = Utils.xml_Utils.getChildAttributebyParentTag(testsuite_filepath, 'Details', 'type', 'Max_Attempts')
        Number_Attempts = Utils.xml_Utils.getChildAttributebyParentTag(testsuite_filepath, 'Details', 'type', 'Number_Attempts')
    ...
    if test_suite_status == False and ts_onError_action and ts_onError_action.upper() == 'ABORT_AS_ERROR':
