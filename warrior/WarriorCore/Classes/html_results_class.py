...
__author__ = 'Sathyamoorthy Radhakrishnan'
...
class WarriorHtmlResults():
    ...
    def html_from_junit(self):
        ...
        for project_node in project_node_list:
            ...
            # for each project node in project_node_list get its testsuite
            # node list.
            testsuite_node_list = project_node.findall("testsuite")
            for testsuite_node in testsuite_node_list:
                # for each testsuite in testsuite node list create testsuite
                # records.
                self.create_testsuite_records(testsuite_node)
                # for each testsuite node in testsuite node list get its
                # testcase list.
                testcase_node_list = testsuite_node.findall("testcase")
                # for each testcase in testcase node list create testcase
                # record.
                for testcase_node in testcase_node_list:
                    ...
                    for step_num, kw_node in enumerate(kw_node_list, 1):
                        tc_details['step_num'] = str(step_num)
    ...
    def create_project_records(self, project_node):
        ...
        if self.table is not None and\
           project_node.get("display") != "False":
            ...
            project_record, project_summary = self._generic_create_records(
             project_node, **details)
    ...
    def create_testsuite_records(self, testsuite_node):
        ...
        if self.table is not None and\
           testsuite_node.get("display") != "False":
            ...
            testsuite_record, testsuite_summary = self._generic_create_records(
             testsuite_node, **details)
    ...
    def create_testcase_record(self, testcase_node):
        ...
        if self.table is not None:
            ...
            testcase_record, testcase_summary = self._generic_create_records(
             testcase_node, **details)
    ...
    def _generic_create_records(self, node, **kwargs):
        ...
        try:
            ...
            row = xml_Utils.create_element("tr", bgcolor=bgcolor,
                                           name=row_name)
            self._create_common_details(row, node, filetype,
                                        tc_details=tc_details)
    ...
    def _create_common_details(self, parent_node, junit_node,
                               filetype, **kwargs):
        ...
        type_map = {"Project": "Project", "Testsuite": "Suite",
                    "Testcase": "Case", "Step": "Step"}
        status = junit_node.get("status")
        span = "1" if filetype == "Step" else "2"
        for item, value in common_details.items():
            td_element = xml_Utils.create_element("td", rowspan=span)
            element = xml_Utils.create_element("div",
                                               style="word-wrap: "
                                               "normal; width: 100px")
            self._set_col_width(element, item)
            text = type_map.get(filetype) if item == "type" else \
                junit_node.get(value)
            self._set_color(element, str(text))
            if item == "type" and value == "Step":
                text = value + " " + kwargs['tc_details']["step_num"]
            if item == "name" and filetype != "Step":
                ...
                if str(status).lower() != "skipped":
                    results_href = self._link_to_results(junit_node,
                                                         filetype, **kwargs)
                    ...
                    defects_href = self._link_to_defects(junit_node,
                                                         filetype, **kwargs)
    ...
    def _set_col_width(self, element, item):
        ...
        element.set("style",
                    "word-wrap: break-word; width: {0}px".format(col_width))

    def _set_color(self, element, text):
        """ Set the color based on item name """
        text_values = {"pass": "#49E20E",
                       "fail": "#FF0000",
                       "error": "#FF0000",
                       "skipped": "#F1C40F",
                       "exception": "#FF0000"}.get(text.lower())
        element.set("style",
                    "word-wrap: break-word; color: {0}".format(text_values))
    ...
    def _link_to_logs(self, junit_node, file_type):
        ...
        if file_type == "Testcase":
            ...
            href_link = xml_Utils.create_element("span",
    ...
    def _link_to_defects(self, junit_node, filetype, **kwargs):
        ...
        if filetype == "Step" and str(status).lower() in failing_status:
            tc_details = kwargs.get("tc_details", {})
    ...
    def _get_keyword_info(self, junit_node,
        ...
        for i in range(len(name_list)):
            subelem = xml_Utils.create_element("span")
            subelem.text = "{0}. {1} = {2}".format(str(i+1),
    ...
    def _get_arglist(self, resultfile):
        ...
        args = xml_Utils.getChildElementWithSpecificXpath(
         resultfile, "./Keyword/Arguments")
        args_list = args.findall("argument")
        return args_list

    def _get_resultfile(self, junit_node, filetype, **kwargs):
        """ get result file for test case/keywords"""
        filepath = "None"
        filename = junit_node.get("name")
        if filetype == "Testcase":
            resultsdir = junit_node.get("resultsdir")
            filepath = str(resultsdir) + os.sep + "Results" + os.sep + \
                str(filename) + "_results.xml"
        elif filetype == "Step":
    ...
    def output_html(self):
        ...
        script/style sheets/bootstrap files to results folder """
        tree = ET.ElementTree(self.html_root)
        tree.write(self.html_results_path, method='html')
        print_info("++++ Results Summary ++++")
        print_info("Open the Results summary file given below in "
                   "a browser to view results summary for this execution")
        print_info("Results summary file: {0}".format(self.html_results_path))
        print_info("+++++++++++++++++++++++++")