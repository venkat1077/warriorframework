    ...
    for testdata in root.findall("testdata"):
        ...
        if testdata.get("execute") == "yes" and exec_flag:
            testdata_key="{0}{1}".format(testdata.get('title',""), \
                                         _get_row(testdata))
            details_dict = _get_cmd_details(testdata, global_obj, system_name, varconfigfile, var_sub=var_sub)
        ...
        if param == "verify_list":
            ...
            for values in vfylist:
                if values is not None and "$" in values:
                    vfylist, maplist = _substitute_var_sub(vfylist, global_obj, varconfigfile, values)
