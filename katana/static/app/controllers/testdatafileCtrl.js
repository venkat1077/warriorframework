...
/**
 * Created by SKULKARN on 8/8/2016.
 */
app.controller('testdatafileCtrl', ['$scope', '$http', '$location', 'fileFactory', function($scope, $http, $location, fileFactory) {

    $scope.xml = {};
    $scope.xml.testdatafiles = [];
    $scope.xml.testdatafolders = [];
    $scope.dir_path = "none";
                                            ...
                                            return false
                                              ...
                                              if(inputValue.indexOf("/") !== -1){
                                                ...
                                                fileFactory.checkfilepath(test_value)
                                                    ...
                                                    function(data) {
                                                        if(data["exists"] == "yes"){
                                                            fileFactory.updateconfigfromtab("testdata", test_value)
                                                            .then(
                                                                function(data) {
                                                                    if(data["updated"] == "yes") {
                                                                        readConfig();
    ...
    $scope.testDataFileChanged = function(element) {
    ...
    function readTestDataFolderNames(directory) {
        $http.get('/testdatafoldernames/' + directory)
            .success(function(data, status, headers, config) {
                $scope.xml.testdatafolders = data;
    ...
    function readTestDataFileNames(directory) {
        $http.get('/testdatafilenames/' + directory)
            .success(function(data, status, headers, config) {
                $scope.xml.testdatafiles = data;
                console.log('testdatafilenames: ' + JSON.stringify(data));
            })
            .error(function(data, status, headers, config) {
    ...
    $scope.navigateToCreateTestDataFilePage = function() {
        console.log('in');
        $location.path('/newtestDatafile');
    };
    ...
    $scope.getSubFilesAndFolders = function(directory){
        ...
        readTestDataFolderNames($scope.dir_path);
        readTestDataFileNames($scope.dir_path);
    };
    ...
    $scope.getParentFilesAndFolders = function(){
        ...
        readTestDataFolderNames(temp_dir_path);
        readTestDataFileNames(temp_dir_path);
    };
