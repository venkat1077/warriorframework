...
//alert("c123");
        ...
        $scope.idf_storePaths = function() {
            ...
            if ($scope.cfg.xmldir.indexOf('/') === -1) {
                idf_tc_folder_array = $scope.cfg.xmldir.split("\\");
            }
            else {
                idf_tc_folder_array = $scope.cfg.xmldir.split("/");
            }
            for (var i = 0; i < idf_tc_folder_array.length; i++) {
                if (idf_tc_folder_array[i] === $scope.idf_path_array[$scope.idf_path_array.length - 1]) {
                    idf_tc_folder_array.splice(i, (idf_tc_folder_array.length - i));
                    break;
                }
            }
            for (i = $scope.idf_path_array.length - 1; i >= 0; i--) {
                idf_tc_folder_array.push($scope.idf_path_array[i])
            }
            if ($scope.testsuitedir.indexOf('/') === -1) {
                idf_data_folder_array = $scope.testsuitedir.split("\\");
            }
            else {
                idf_data_folder_array = $scope.testsuitedir.split("/");
            }
            for (i = 0; i < idf_data_folder_array.length; i++) {
                if (idf_tc_folder_array[i] !== idf_data_folder_array[i]) {
                    idf_folder_index = i;
                }
            }
            if (idf_folder_index !== -1) {
                var dots = idf_data_folder_array.length - idf_folder_index;
                for (i = 0; i < dots; i++) {
                    idf_final_array.push("..");
                }
            } else {
                idf_folder_index = idf_data_folder_array.length;
            }
            for (i = idf_folder_index; i < idf_tc_folder_array.length; i++) {
                idf_final_array.push(idf_tc_folder_array[i]);
            }
        ...
        $scope.td_df_storePaths = function(index){
            ...
            if($scope.cfg.xmldir.indexOf('/') === -1) {
                td_df_folder_array = $scope.cfg.xmldir.split("\\");
            }
            else{
                td_df_folder_array = $scope.cfg.xmldir.split("/");
            }
       ...
       // ChildCtrl.baseCtrl.readConfig();
           ...
           var finalJSON = {
            ...
            if ($scope.savecreateTestsuite == true || $scope.savecreateTestsuiteCap == false) {
                $route.reload();
                   ...
                   $location.path('/newtestsuite/__new__/none');
            }  else {
        ...
        $timeout(function() {
            ...
            $scope.Date = $scope.suitemodel.TestSuite.Details.Date;
            $scope.suitemodel.TestSuite.Details.Time = ChildCtrl.baseCtrl.getTime();
            $scope.Time = $scope.suitemodel.TestSuite.Details.Time;
