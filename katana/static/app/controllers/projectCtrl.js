...
app.controller('projectCtrl', ['$scope', '$http', '$location', 'fileFactory', function($scope, $http, $location, fileFactory) {

    $scope.xml = {};
    $scope.xml.projectfiles = [];
    $scope.xml.projectfolders = [];
    $scope.dir_path = "none";
    ...
    function readConfig(){
        $http.get('/readconfig')
            .success(function(data, status, headers, config) {
                var temp = data["projdir"];
                $scope.base_dir = temp.split('\\').pop().split('/').pop();
                if(temp == ""){
                    setTimeout(function(){
                        swal({
                            title: "The Configuration for the Project Directory hasn't been set up yet. Do you want to set it up?",
                            ...
                            function(isConfirm){
                                if (isConfirm) {
                                    setTimeout(function(){
                                        swal({
                                            title: "The Project Directory Configuration",
                                            text: "Location of the Project Directory:",
                                            type: "input",
                                            showCancelButton: true,
                                        ...
                                        function(inputValue){
                                          ...
                                          if (inputValue === "") {
                                            swal.showInputError("You should enter the location of the Project Directory.");
                                            return false
                                          }
                                          else{
                                              if(inputValue.indexOf("/") !== -1){
                                                ...
                                                fileFactory.checkfilepath(test_value)
                                                    ...
                                                    function(data) {
                                                        if(data["exists"] == "yes"){
                                                            fileFactory.updateconfigfromtab("projdir", test_value)
                                                            .then(
                                                                function(data) {
                                                                    if(data["updated"] == "yes") {
                                                                        readConfig();
                                                                        readProjectFolderNames("none");
                                                                        readProjectFileNames("none");
                                                                        swal({
                                                                            title: "Saved the Project Directory Path: ",
                                                                            text: inputValue,
                                                                            timer: 1250,
                                                                            type: "success",
    ...
    function readProjectFileNames(directory) {
        $http.get('/projectfilenames/' + directory)
            .success(function(data, status, headers, config) {
                $scope.xml.projectfiles = data;
                console.log('projectfilenames: ' + JSON.stringify(data));
            })
            .error(function(data, status, headers, config) {
    ...
    function readProjectFolderNames(directory) {
        $http.get('/projectfoldernames/' + directory)
            .success(function(data, status, headers, config) {
                $scope.xml.projectfolders = data;
                console.log('projectfoldernames: ' + JSON.stringify(data));
            })
            .error(function(data, status, headers, config) {
                alert("Error fetching project folder names.", status, headers);
            });
    }
...
/*    $scope.navigateToCreateProjectPage = function() {
    ...
    $scope.getSubFilesAndFolders = function(directory){
        ...
        readProjectFolderNames($scope.dir_path);
        readProjectFileNames($scope.dir_path);
    };
    ...
    $scope.getParentFilesAndFolders = function(){
        ...
        readProjectFolderNames(temp_dir_path);
        readProjectFileNames(temp_dir_path);
    };
